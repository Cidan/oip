...
type MainDriver struct {
	...
	BaseDir      string      // Bucket in this context
...
func (driver *MainDriver) GetTLSConfig() (*tls.Config, error) {
	if driver.tlsConfig == nil {
		if cert, err := tls.LoadX509KeyPair("certs/mycert.crt", "certs/mycert.key"); err == nil {
...
func (driver *MainDriver) ListFiles(cc server.ClientContext) ([]os.FileInfo, error) {
	...
	bucket := driver.GCSClient.Bucket(driver.BaseDir)
	objs := bucket.Objects(context.Background(), nil)
...
func NewGCSDriver() (*MainDriver, error) {

	driver := &MainDriver{
		SettingsFile: "conf/settings.toml",
		BaseDir:      "lbd_scrap",
	}
...
func main() {
	...
	ftpServer := server.NewFtpServer(d)
